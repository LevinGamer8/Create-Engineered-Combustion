 ...
 * not a change to the shape of anything. With the defaults below the two windows are
 * exactly adjacent and both reach zero lift at 540 degrees, so no overlap exists.
 */
public enum ValveTiming {
	 ...
	 *
	 * <p>Its opening point is also the point at which a cylinder becomes able to fire
	 ...
	 * </pre>
	 *
	 * <p>A raised cosine, chosen for one property above all: <b>both its value and its
	 * slope are zero at each end</b>. A valve that snapped open would read as a
	 * flickering block rather than a mechanism, and one that merely reached zero with a
	 * non-zero slope - a triangle, or a half-sine - would visibly kink as it met the
	 * seat. This meets the seat the way a cam does.
	 *
	 ...
	 * gas-flow model: this is the shape a player reads as "a cam is pushing that open".
	 *
